  ...
  useEffect(() => {
    const fetchUserProgresses = async () => {
      try {
        ...
        const data = response.data.sort((a, b) => b.scores - a.scores).slice(0, 100)
    ...
    <div>
      ...
      <div className="min-h-screen w-screen bg-gray-100 top-0 absolute">
        ...
        <div className="max-w-7xl mx-auto px-2 md:pl-20 overflow-hidden">
          <div className={`flex flex-row w-full gap-1 transition ease-in-out duration-300`}>
            <div className="flex-auto relative min-w-full lg:min-w-0">
              ...
              <div className="relative bg-white backdrop-blur-sm rounded-3xl p-8 border border text-gray-900">
                <div className="space-y-3">
                  <div className="flex flex-row items-center">
                    <div className="font-semibold w-48">Position</div>
                    <div className="font-semibold w-full">Name</div>
                    <div className="font-semibold w-48 text-center">Total Scores</div>
                  </div>

                  {!loading && userProgresses.map((userProgress, index) => {
                    return (
                      <>
                        ...
                        <div className="flex flex-row items-center">
                          ...
                          <div className="font-semibold w-full">{userProgress.user.username}</div>
                          <div className="font-semibold w-48 text-center">{userProgress.scores}</div>
                        </div>
                      </>
                    );
                  })}
